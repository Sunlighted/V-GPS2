...
class OctoLCEncodingWrapper(nn.Module):
    octo_encoder: nn.Module
    ...
    def __call__(
        ...
        imgs = observations["image"]  # (B, H, W, 3) 或 (B, T, H, W, 3)

        if imgs.ndim == 5:
            # (B, T, H, W, C) 直接认为 T = horizon
            B, T = imgs.shape[:2]
            images = imgs        # (B, T, H, W, C)
        else:
            # (B, H, W, C) -> (B, 1, H, W, C)
            B = imgs.shape[0]
            T = 1
        ...
        # timestep_pad_mask: (B, T)
        timestep_pad_mask = jnp.ones((B, T), dtype=bool)

        # Octo 的 observations dict
        octo_observations = {
...
class PrecomputedFeatureEncodingWrapper(nn.Module):
    """Identity encoder for precomputed embeddings (e.g., TTT outputs)."""
