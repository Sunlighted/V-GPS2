...
class EmbeddingCQLCrossAttenAgent(SACAgent):
    @overrides
    ...
    def get_q_values(self, observations, goals, actions):
        obs_payload = self._format_goal_conditioned_obs(observations, goals)
    ...
    def get_eval_values1(self, traj, seed, goals, max_chunk_size=16):
        ...
        for start in range(0, T, max_chunk_size):
            ...
            goal_chunk["language"] = goal_chunk["language"][:, 0, :]
            ...
            # 3) 当前 critic Q
            chunk_obs = self._format_goal_conditioned_obs(obs_chunk, goal_chunk)
        ...
        **kwargs,
        ...
        if config.use_precomputed_embeddings:
            ...
            encoder_def = OctoLCEncodingWrapper(
                octo_encoder=OctoEncoderModule(octo_module=octo_model.module, readout_name="action", pool_type="mean"),
                use_proprio=False,
                stop_gradient=True,
            )
        ...
        init_rngs = {'params': init_rng, 'dropout': dropout_key}
