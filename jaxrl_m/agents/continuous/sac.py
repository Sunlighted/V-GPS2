...
class SACAgent(flax.struct.PyTreeNode):
    ...
    def _include_goals_in_obs(self, batch, which_obs: str):
        ...
        obs = batch[which_obs]
        return self._format_goal_conditioned_obs(obs, batch["goals"])

    def _format_goal_conditioned_obs(
        self, observations: Data, goals: Optional[Data] = None
    ) -> Union[Data, Tuple[Data, Data]]:
        """Return the correct observation payload based on goal conditioning."""
        if self.config["goal_conditioned"]:
            if goals is None:
                raise ValueError(
                    "Goal-conditioned agents require `goals` to be provided."
                )
    ...
    def get_debug_metrics(self, batch, **kwargs):
        """for compatibility with other agents and training scripts"""
        return {}
