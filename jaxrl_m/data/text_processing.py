...
class T5Tokenizer(TextProcessor):
    def __init__(
        self,
        ...
        self.tokenizer_kwargs = tokenizer_kwargs
        self.model = FlaxT5EncoderModel.from_pretrained(tokenizer_name)
    ...
    #         **self.tokenizer_kwargs,
    #     )
    #     return np.array(self.model(**inputs).last_hidden_state).reshape(B, -1)
    def encode(self, strings):
        inputs = self.tokenizer(strings, **self.tokenizer_kwargs)
        last_hidden = np.array(self.model(**inputs).last_hidden_state)  # [B, seq_len, hidden_dim]
        return last_hidden
...
class ClipTokenizer(TextProcessor):
    def __init__(
        self,
        ...
        self.tokenizer_kwargs = tokenizer_kwargs
        self.model = FlaxCLIPTextModel.from_pretrained(tokenizer_name)
