...
from jaxrl_m.agents.continuous.cql_ttt_predict import CQLTTTPredictAgent
from jaxrl_m.networks.simsiam_networks import cosine_similarity_loss

FLAGS = flags.FLAGS

flags.DEFINE_string("name", "", "Experiment name.")
flags.DEFINE_string("project", "cql_ttt_predict", "WandB project name.")
...
def build_ttt_adaptation_fn(agent: CQLTTTPredictAgent):
    """
    Build TTT adaptation function that updates BOTH the encoder AND TTT components.

    IMPORTANT: The encoder is now a standalone module at modules_encoder.
    It is shared by actor/critic/ttt through the _encode() method.
    To affect Q-values during TTT, we update modules_encoder and modules_ttt.

    Args:
        agent: The CQLTTTPredictAgent

    Returns:
        Adaptation function that returns ((adapted_encoder, adapted_ttt), losses)
    """
    ...
    lambda_ttt = config.get("lambda_ttt", 2.0)
    ...
    def _adapt_jit(full_params, batch, lr: float, num_steps: int):
        ...
        w.r.t. encoder (modules_encoder) and ttt_module components (modules_ttt_module).
        ...
        def compute_loss(params):
            """Compute TTT loss using the encoder + TTT module (P_K -> f_adapt).

            This mirrors the logic in `cql_ttt_predict.py:ttt_loss_fn`:
            1. encode obs and next_obs via the shared encoder
            2. compute P_K(concat(z_t, action)) via the ttt_module's P_K
            3. run f_adapt on the projection to predict next latent
            4. cosine similarity loss between pred and (stop_grad) z_tp1
            """
            # Prepare inputs
            ...
            # Step 2: Build combined input and project with P_K (use module method)
            # Call the ttt_module's P_K via apply_fn.method
            pred = apply_fn(
                variables,
                z_t,
                actions,
            ...
            # Step 4: target is next latent (optionally stop_grad)
            if stop_grad_target:
            ...
            loss = cosine_similarity_loss(pred, target, normalize=normalize_latents)

            return lambda_ttt * loss
        ...
        def adapt_step(i, carry):
            ...
            # Only update f adapt params (modules_ttt_module)
            ...
            def make_update_mask(params):
                ...
                def _mask_fn(path, _):
                    ...
                    # Update encoder (modules_encoder) and ttt_module f_adapt
                    # if "modules_encoder" in path_str:
                    #     return 1.0
                    if "modules_ttt_module/f_adapt" in path_str:
                        return 1.0
                    return 0.0
    ...
    def adapt(frozen_params, batch, lr: float, num_steps: int):
        """
        Run multiple TTT adaptation steps.

        Args:
            ttt_module_params: Initial ttt_module params (unused, kept for API compat)
        ...
        Returns:
            (adapted_encoder_params, adapted_ttt_module_params), losses
        """
        # Run adaptation on full params
        adapted_full_params, losses = _adapt_jit(frozen_params, batch, lr, num_steps)

        # Extract the adapted encoder and ttt_module params
        # Encoder is now at modules_encoder (not modules_actor/encoder)
        adapted_encoder = adapted_full_params.get("modules_encoder", {})
        adapted_ttt_module = adapted_full_params.get("modules_ttt_module", {})

        return (adapted_encoder, adapted_ttt_module), losses

    return adapt


def _update_agent_ttt(agent: CQLTTTPredictAgent, adapted_params):
    """
    Update agent's f adapt parameters (for TTT).

    Args:
        agent: The agent to update
        adapted_params: Tuple of (adapted_encoder_params, adapted_ttt_module_params)
                       OR just adapted_ttt_module_params for backward compatibility
    """
    if adapted_params is None:
        logging.warning("TTT adaptation returned None params, skipping update")
        return agent

    # We only want to update the TTT/SimSiam (f_adapt) parameters during TTT;
    # do NOT replace or modify the encoder parameters here.
    # Handle both new format (encoder, ttt_module) and old format (ttt_module only)
    if isinstance(adapted_params, tuple) and len(adapted_params) == 2:
        adapted_encoder, adapted_ttt = adapted_params
        adapted_encoder = None  # ignore encoder on purpose
    else:
        adapted_ttt = adapted_params

    if adapted_ttt is None:
        logging.warning("adapted_ttt is None, skipping ttt update")
        return agent

    # Ensure ttt params are frozen
    if not isinstance(adapted_ttt, flax.core.FrozenDict):
        adapted_ttt = flax.core.freeze(dict(adapted_ttt))

    # Build new params dict
    new_params_dict = dict(agent.state.params)
    new_params_dict["modules_ttt_module"] = adapted_ttt
    ...
    new_target_dict["modules_ttt_module"] = adapted_ttt
...
def plot_value_overlay(
    agent_adapted: CQLTTTPredictAgent,
    traj: Dict[str, Any],
    agent_base: CQLTTTPredictAgent,
...
def run_ttt_adaptation_on_trajectory(
    ...
    Now adapts BOTH the encoder (in modules_actor) AND ttt_module components.
    This is necessary because Q-values depend on the encoder.

    Args:
        agent: Base agent
        traj: Trajectory dict
        ttt_adapt_fn: TTT adaptation function (returns (encoder, ttt_module), losses)
    ...
    Returns:
        Agent with adapted encoder and ttt_module module
    """
    ...
    base_ttt_params = agent.state.params.get("modules_ttt_module")
    base_encoder_params = agent.state.params.get("modules_encoder")

    if base_ttt_params is None:
        logging.error("No 'modules_ttt_module' key in agent params for trajectory adaptation! Keys: %s",
    ...
    logging.info("  Before: %s", _param_stats(base_ttt_params, "ttt"))
    logging.info("  Before: %s", _param_stats(base_encoder_params, "encoder"))

    all_losses = []

    # Helper to rebuild full params with current encoder and ttt for online mode
    def _rebuild_full_params(current_encoder, current_ttt):
        """Rebuild agent params with updated encoder and ttt for next iteration."""
        new_params = dict(agent.state.params)
        new_params["modules_encoder"] = current_encoder
        new_params["modules_ttt_module"] = current_ttt
        return flax.core.freeze(new_params)

    # Alias for backward compat
    def _rebuild_params_with_encoder(current_encoder):
        return _rebuild_full_params(current_encoder, base_ttt_params)
    ...
    if mode == "online":
        ...
        for t in range(T - 1):  # T-1 because we need (obs_t, obs_{t+1}, action_t)
            ...
            (current_encoder, current_ttt), losses = ttt_adapt_fn(
                current_full_params,
                step_batch,
                ttt_lr,
                ttt_steps,
            ...
            # Rebuild full params with updated encoder AND ttt for next iteration
            current_full_params = _rebuild_full_params(current_encoder, current_ttt)

        adapted_params = (current_encoder, current_ttt)
    ...
    # adapted_params is now (encoder, ttt) tuple
    adapted_encoder, adapted_ttt = adapted_params
    logging.info("  After:  %s", _param_stats(adapted_ttt, "ttt"))
    logging.info("  After:  %s", _param_stats(adapted_encoder, "encoder"))

    # Compute param difference for ttt
    ...
    sim_diff_norm, sim_diff_max = _param_diff(adapted_ttt, base_ttt_params)
    ...
    return _update_agent_ttt(agent, adapted_params)
...
def main(_):
    ...
    train_datasets_kwargs_list, train_sample_weights = _safe_filter_datasets(
        FLAGS.oxedata_config["dataset_kwargs_list"],
        FLAGS.oxedata_config["sample_weights"],
        ["fractal20220817_data", "bridge_dataset"],
    )
    ...
    logging.info("Created CQLTTTPredictAgent")
    logging.info(f"  lambda_ttt: {agent.config.get('lambda_ttt', 0.0)}")
    logging.info(f"  shared_encoder: {agent.config.get('shared_encoder', True)}")
    logging.info(f"  param keys: {list(agent.state.params.keys())}")
    def log_param_shapes(params):
        logging.info("--- 参数详细列表 ---")
        def _log_info(path, leaf):
            path_str = "/".join(str(p.key if hasattr(p, 'key') else p) for p in path)
            logging.info(f"Key: {path_str:<60} | Shape: {leaf.shape}")
        
        jax.tree_util.tree_map_with_path(_log_info, params)

    # 直接调用
    log_param_shapes(agent.state.params)
    ...
    ttt_enabled = (
        ...
        agent.config.get("lambda_ttt", 0.0) > 0
    )
    ...
    for step in tqdm.tqdm(range(int(FLAGS.config.num_steps)), desc="Training"):
        ...
        try:
            ...
            if (step + 1) % FLAGS.config.log_interval == 0:
                ...
                rl_metrics = {k: v for k, v in metrics.items() if not k.startswith("ttt_module/")}
                ttt_module_metrics = {k: v for k, v in metrics.items() if k.startswith("ttt_module/")}

                wandb_logger.log({"training/rl": rl_metrics}, step=step)
                if ttt_module_metrics:
                    wandb_logger.log({"training/ttt_module": ttt_module_metrics}, step=step)
            ...
            if (step + 1) % FLAGS.config.eval_interval == 0:
                ...
                # Store base ttt_module params for TTT
                base_ttt_params = agent.state.params.get("modules_ttt_module")
                if base_ttt_params is None:
                    logging.error("No 'modules_ttt_module' key in agent params! Keys: %s", list(agent.state.params.keys()))
                online_ttt_params = base_ttt_params
                ...
                for eval_idx in range(num_eval_batches):
                    ...
                    if ttt_enabled and ttt_adapt_fn is not None:
                        ...
                        start_params = (
                            base_ttt_params if ttt_mode == "standard"
                            else online_ttt_params
                        )

                        # Get frozen params (everything except ttt)
                        ...
                        online_ttt_params = adapted_params

                        # Create eval agent with adapted ttt
                        eval_agent = _update_agent_ttt(agent, adapted_params)
                    ...
                    for plot_idx in range(num_value_plots):
                        try:
                            ...
                            if ttt_enabled and ttt_adapt_fn is not None:
                                ...
                                # Compare encoder outputs (ttt_module module)
                                ...
                                # Forward through ttt_module for base agent
                                try:
                                    # ttt_base = agent.state.apply_fn(
                                    ...
                                    ttt_adapted = agent_adapted_online.state.apply_fn(
                                    ...
                                    pred_base = np.asarray(ttt_base)
                                    pred_adapted = np.asarray(ttt_adapted)
                                    pred_diff = pred_adapted - pred_base
                                    logging.info("  TTT prediction comparison:")
                                    ...
                                    logging.warning("  Could not compare ttt outputs: %s", e)

                                # Compare ttt params directly
                                base_ttt = agent.state.params.get("modules_ttt_module", {})
                                adapted_ttt = agent_adapted_online.state.params.get("modules_ttt_module", {})
                                if base_ttt and adapted_ttt:
                                    base_flat = jax.tree_util.tree_leaves(base_ttt)
                                    adapted_flat = jax.tree_util.tree_leaves(adapted_ttt)
                                ...
                                if base_ttt:
                                    logging.info("  modules_ttt keys: %s", list(base_ttt.keys()))
                                    for k, v in base_ttt.items():
                                        if hasattr(v, 'keys'):
