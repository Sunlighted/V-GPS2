...
def get_octo_embeddings_batch(octo_model, images, language_list):
    ...
    images_octo = images[:, None, :, :, :]  # (B, 1, 256, 256, 3)
    ...
    embeddings = readout_tokens[:, 0].mean(axis=1)  # Mean over tokens
    
    return embeddings


def process_batch_with_octo(batch, octo_model, text_processor=None):
    """
    ...
    # Encode with OCTO
    all_embeddings = get_octo_embeddings_batch(octo_model, stacked_images, stacked_language)
...
def process_trajectory_for_eval(traj_batch, octo_model):
    ...
    obs_embeddings = get_octo_embeddings_batch(octo_model, images, language_list)  # (T, D)
