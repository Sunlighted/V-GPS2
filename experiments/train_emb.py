...
from octo.data.dataset_em import make_interleaved_dataset
from octo.data.dataset_em import make_dataset_from_rlds, apply_trajectory_transforms
from octo.data.oxe import make_oxe_dataset_kwargs_and_weights
from octo.utils.train_callbacks_em import create_validation_dataset
...
def main(_):
    ...
    def process_text(batch):
        if text_processor is not None:
            ...
            batch["goals"]["language"] = np.array(
                [
                    np.zeros(1, dtype=np.int32)
                    for _ in range(batch["goals"]["language"].shape[0])
                ]
            )
        return batch
    ...
    if "fractal" in oxe_kwargs.data_mix or "oxe" in oxe_kwargs.data_mix or "rtx" in oxe_kwargs.data_mix:
        ...
        # [修改 1] .iterator() -> .as_numpy_iterator()
        val_traj_data_fractal_iter = map(process_oxe_batch, val_data_fractal.shuffle(1000).repeat().as_numpy_iterator())

        # [修改 2] .iterator() -> .as_numpy_iterator()
        val_data_fractal_iter = map(
            shard_fn, map(process_oxe_batch, val_data_fractal.as_numpy_iterator())
        )

        # [修改 3] .iterator(prefetch=0) -> .as_numpy_iterator()
        # 注意：TF 原生 iterator 没有 prefetch 参数，如果需要 prefetch，要在前面写 .prefetch(N)
        # 但对于 validation 这里设为 0 也就意味着不需要显式 prefetch
    ...
    # [修改 4] .iterator() -> .as_numpy_iterator()
    val_traj_data_iter = map(process_oxe_batch, val_data.shuffle(1000).repeat().as_numpy_iterator())

    # [修改 5] .iterator() -> .as_numpy_iterator()
    # 改正为：
    val_data_iter = map(
        shard_fn, map(process_oxe_batch, val_data.as_numpy_iterator())
    )

    # [修改 6] .iterator(prefetch=0) -> .as_numpy_iterator()
    val_data_iter = (
            val_data.unbatch()
            .shuffle(1000)
            .repeat()
    ...
    def background_prefetch(iterator, queue_size=2):
        # 一个简单的后台预取器
    ...
    # [修改 7 - 重点] train_data 这里也要改！
    # 原代码: train_data.iterator(prefetch=10)
    # 修改后: train_data.prefetch(10).as_numpy_iterator()
    # 解释: 原生 Dataset 没有 iterator 参数，必须先调用 .prefetch 方法，再转 numpy 迭代器
    processed_stream = map(process_oxe_batch, train_data.prefetch(10).as_numpy_iterator()) 
    
    sharded_stream = map(shard_fn, processed_stream)

    # 最后加上后台预取，这样 Python 处理和 GPU 计算就可以并行了
    train_data_iter = background_prefetch(sharded_stream, queue_size=4)
