...
def make_single_traj_dataset(
    ...
    *,
    train: bool,
    max_T: int = 120,  # 新增：最大步数约束
    traj_transform_kwargs: dict = {},
    frame_transform_kwargs: dict = {},
) -> dl.DLataset:
    """创建单个数据集。返回一个每项为 (max_T, D) 结构的轨迹数据集。"""
    ...
    # 2. 立即过滤：剔除长度大于 120 的轨迹
    # 这一步放在最前面，可以节省后续所有变换的计算开销
    dataset = dataset.filter(lambda x: tf.shape(x["action"])[0] <= max_T)

    # 3. 应用轨迹级变换
    # 注意：确保 traj_transform_kwargs 里的 window_size 被设为 1
    dataset = apply_trajectory_transforms(
        dataset, 
        **traj_transform_kwargs, 
        train=train
    ...
    # 4. 补齐至固定长度 max_T 并添加 pad_mask
    # 使用之前定义的 pad_trajectory_to_max 函数
    dataset = dataset.traj_map(
        partial(pad_trajectory_to_max, max_T=max_T),
        num_parallel_calls=tf.data.AUTOTUNE
    )

    # 5. 应用帧变换 (包括图像解码、Resize 等)
    # 此时 dataset 的元素形状已经是 (max_T, ...)
...
def make_interleaved_traj_dataset(
    dataset_kwargs_list: Sequence[dict],
    sample_weights: Optional[Sequence[float]] = None,
    *,
    train: bool,
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset = dataset.filter(lambda x: tf.shape(x["action"])[0] <= 120)
