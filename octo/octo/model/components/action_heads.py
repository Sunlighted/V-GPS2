...
class ContinuousActionHead(nn.Module, ActionHead):
    ...
    # def get_embedding(self, transformer_outputs: Dict[str, TokenGroup], train: bool = False) -> jax.Array:
    #     """
    #     Returns the pooled embedding from the transformer, before the final projection.
    #     """
    #     token_group = transformer_outputs[self.readout_key]
    #     assert token_group.tokens.ndim == 4, (
    #         f"Expected token_group.tokens to have shape (batch_size, window_size, num_tokens, embedding_size), "
    ...
    #     if self.use_map:
    #         embeddings = self.map_head(token_group, train=train)[:, :, 0]  # (batch, window, embedding_dim)
    #     else:
    #         embeddings = token_group.tokens.mean(axis=-2)  # mean pooling

    #     return embeddings  # (batch_size, window_size, embedding_dim)
...
class DiscreteActionHead(nn.Module, ActionHead):
    ...
    # def get_embedding(self, transformer_outputs: Dict[str, TokenGroup], train: bool = False) -> jax.Array:
    #     """
    #     Returns the pooled embedding from the transformer, before projecting to vocab logits.
    #     """
    #     token_group = transformer_outputs[self.readout_key]

    #     if self.use_map:
    #         embeddings = self.map_head(token_group, train=train)  # (batch, window, n_tokens, embedding_dim)
    #     else:
    #         embeddings = token_group.tokens
    #         if embeddings.shape[-2] != self.n_tokens:
    #             raise ValueError(f"Expected {self.n_tokens} tokens, got {embeddings.shape[-2]}")

    #     return embeddings  # (batch, window, n_tokens, embedding_dim)
...
class DiffusionActionHead(nn.Module):
    ...
    def predict_step_crepe(
        self,
        transformer_outputs: Dict[str, TokenGroup],
        current_x: jax.Array,    # 形状: (*sample_shape, batch_size, window_size, action_dim_flat)
        time_indices: jax.Array, # 形状: (*sample_shape,) 每个副本的时间步
        rng: PRNGKey,
        train: bool = False,
        embodiment_action_dim: Optional[int] = None,
    ) -> Tuple[jax.Array, jax.Array]:
        """
        CREPE 单步迭代：逻辑和形状处理完全对齐 predict_action。
        """
        # 1. 提取维度信息
        batch_size, window_size = transformer_outputs[self.readout_key].tokens.shape[:2]
        # 根据 current_x 反推 sample_shape (例如 (50,))
        sample_shape = current_x.shape[:-3] 
        module, variables = self.unbind()

        # 2. 准备动作掩码 (完全对齐 predict_action)
        action_mask = jnp.ones(
            (
                *sample_shape,
                batch_size,
        ...
        # print("--- Transformer Outputs Shapes ---")
        # for key, value in transformer_outputs.items():
        #     if hasattr(value, 'tokens'):
        #         print(f"Key: {key:20} | Shape: {value.tokens.shape}")
        #     else:
        #         print(f"Key: {key:20} | No tokens attribute")

        # 4. 准备时间输入
        # time_indices 原始形状: (*sample_shape,)
        # 目标形状: (*sample_shape, batch_size, window_size, 1) 以对齐 current_x
        input_time = time_indices.reshape(sample_shape + (1, 1, 1))
        input_time = jnp.broadcast_to(input_time, (*current_x.shape[:-1], 1))
        ...
        # 6. 提取当前步骤的系数 (需 reshape 以支持广播)
        # 将 (T,) 维度的系数索引并变为 (*sample_shape, 1, 1, 1)
        def get_coeff(coeff_arr):
            return coeff_arr[time_indices].reshape(sample_shape + (1, 1, 1))

        curr_alpha = get_coeff(self.alphas)
        curr_alpha_hat = get_coeff(self.alpha_hats)
        curr_beta = get_coeff(self.betas)

        # 7. 预测清洁动作 a0 (Tweedie 公式)
        pred_a0 = (current_x - jnp.sqrt(1 - curr_alpha_hat) * eps_pred) / jnp.sqrt(curr_alpha_hat)
        pred_a0 = jnp.clip(pred_a0, -self.max_action, self.max_action)

        # 8. 执行 DDPM 去噪步 (严格对齐 scan_fn)
        alpha_1 = 1 / jnp.sqrt(curr_alpha)
        alpha_2 = (1 - curr_alpha) / (jnp.sqrt(1 - curr_alpha_hat))
        next_x = alpha_1 * (current_x - alpha_2 * eps_pred)

        # 9. 添加 Langevin 噪声
        rng, key = jax.random.split(rng)
        z = jax.random.normal(key, shape=current_x.shape)
        
        # 对应 scan_fn 中的 (time > 0) 逻辑
        noise_mask = (time_indices > 0).reshape(sample_shape + (1, 1, 1))
        next_x = next_x + noise_mask * (jnp.sqrt(curr_beta) * z)
        next_x = jnp.clip(next_x, -self.max_action, self.max_action)

        # 10. 应用动作掩码 (处理 masked dimensions)
        next_x = jnp.where(
            flat_action_mask, next_x, jnp.sqrt(1 - curr_alpha_hat) * z
        )

        return pred_a0, next_x
    ...
    #         f"but got shape {token_group.tokens.shape}"
    #     )
    #     print(token_group.tokens.shape)
    #     if self.use_map:  # Multi-head attention pooling
    #         embeddings = self.map_head(token_group, train=train)[:, :, 0]
    #     else:  # mean pooling
    #         embeddings = token_group.tokens.mean(axis=-2)
