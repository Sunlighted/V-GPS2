...
class OctoModel:
    ...
    def create_tasks(
        self, goals: Optional[Data] = None, texts: Optional[Sequence[str]] = None, already_encoded: bool = False
    ):
        ...
        if self.text_processor is not None and not already_encoded:
            tasks["language_instruction"] = self.text_processor.encode(
                tasks["language_instruction"]
            )
        elif self.text_processor is None and "language_instruction" in tasks:
            del tasks["language_instruction"]
        elif already_encoded and isinstance(tasks["language_instruction"], list):
            tasks["language_instruction"] = np.array(tasks["language_instruction"]).reshape(batch_size,-1,768)
